...
def drawBoundingBox(img, rects, thickness = 1, createImg = False):
	for x, y, w, h in rects:
		...
		if (x + w - pad_w) > x + 64 and (y + h - pad_h) > y + 128:    
			...
			#cv2.putText(img, 'box', (x+pad_w, y+pad_h), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,0,255), 2,  cv2.LINE_AA)
			if createImg:
				cropBoundingBox(x+pad_w+thickness, y+pad_h+thickness, x+w-pad_w-thickness, y+h-pad_h-thickness, img, x)

# Using an image histogram and the trained model it returns the the box class
def classifyBoundingBox(imgHist, model):
	from sklearn.linear_model import LogisticRegression
	return model.predict(imgHist)
...
def main():	
	...
	# Load trained model for classification with the current detection system
	from sklearn.externals import joblib
	clf = joblib.load('../Detection/Models/model.sav')
	...
	if capWorking == True:
		...
		while frames < frameCount:
			...
			if detectionMode == True:
				drawBoundingBox(frame, contours, 2, True)
